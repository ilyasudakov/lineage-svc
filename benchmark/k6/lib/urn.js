...
// Match the loaded fixture's dataset pool size: generator uses max(100, edges // 10).
// For a 100k-edge fixture → 10k datasets. Override via URN_POOL.
...
export function randomDatasetUrn() {
  ...
  const name = `${src}.api.${rpt}.${id}`; // matches the fixture's inputs[].name
